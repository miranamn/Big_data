 ...
 *
 * Цель этого упражнения - обогатить поездки на такси информацией о стоимости проезда.
 *
 * Parameters:
...
public class ExpiringStateExercise extends ExerciseBase {
	...
	public static class EnrichmentFunction extends KeyedCoProcessFunction<Long, TaxiRide, TaxiFare, Tuple2<TaxiRide, TaxiFare>> {
		...
		public void open(Configuration config) {
			taxiRideValueState = getRuntimeContext().
					getState(new ValueStateDescriptor<>("save taxi Ride", TaxiRide.class));
			taxiFareValueState = getRuntimeContext().
					getState(new ValueStateDescriptor<>("save taxi Fare", TaxiFare.class));
		}

		@Override
		public void onTimer(long timestamp, OnTimerContext ctx, Collector<Tuple2<TaxiRide, TaxiFare>> out) throws
				Exception {
			if (taxiFareValueState.value() != null) {
				ctx.output(unmatchedFares, taxiFareValueState.value());
				taxiFareValueState.clear();
			}
			if (taxiRideValueState.value() != null) {
				ctx.output(unmatchedRides, taxiRideValueState.value());
				taxiRideValueState.clear();
			}
		}

		@Override
		public void processElement1(TaxiRide ride, Context context, Collector<Tuple2<TaxiRide, TaxiFare>> out) throws
				Exception {
			TaxiFare fare = taxiFareValueState.value();
			if (fare != null) {
				taxiFareValueState.clear();
		...
		@Override
		public void processElement2(TaxiFare fare, Context context, Collector<Tuple2<TaxiRide, TaxiFare>> out) throws
				Exception {
			TaxiRide ride = taxiRideValueState.value();
			if (ride != null) {
				taxiRideValueState.clear();
